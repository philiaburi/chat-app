<template>
  <v-card
    elevation="0"
    class="d-flex flex-column position-relative"
    height="calc(100vh - 200px)"
    max-height="calc(100vh - 200px)"
  >
    <ul class="d-flex flex-column px-0 py-12 px-6" v-if="talkUserId">
      <li
        v-for="item in messages"
        :key="item.id"
        class="my-2 d-flex flex-column"
        :class="item.user_id === userId ? 'ml-auto' : 'mr-auto'"
      >
        <span
          class="text-caption"
          :class="item.user_id === userId ? 'text-right' : 'text-left'"
          >{{ item.name }}</span
        >
        <span
          class="pa-2 text-subtitle-2 chat-box text-start"
          :class="item.user_id === userId ? 'my-chat-box' : 'other-chat-box'"
        >
          {{ item.message }}
        </span>
      </li>
    </ul>
    <div class="d-flex bottom-bar mt-auto px-2 py-3" v-if="talkUserId">
      <v-text-field
        v-model="sendMessageText"
        append-outer-icon="mdi-send"
        single-line
        outlined
        hide-details
        dense
        label="メッセージ"
        type="text"
        @click:append-outer="sendMessage"
      ></v-text-field>
    </div>
    <!-- 会話する相手がいない場合 -->
    <div class="ma-auto text-heading-6" v-if="!talkUserId">
      会話する相手を選ぼう！
    </div>
  </v-card>
</template>

<script lang="ts">
import Vue from "vue";
import { db } from "../plugins/firebase";
import {
  query,
  where,
  onSnapshot,
  collection,
  getDocs,
  addDoc,
  orderBy,
} from "firebase/firestore";

export default Vue.extend({
  props: {
    userId: {
      type: String,
      required: true,
    },
    userName: {
      type: String,
      required: true,
    },
    talkUserId: {
      type: String,
      required: true,
    },
  },
  data() {
    return {
      sendMessageText: "",
      messages: [],
      roomId: null,
    };
  },
  watch: {
    talkUserId(id) {
      if (!id) return;
      this.sendMessageText = "";
      this.messages = [];
      this.roomId = null;
      this.setMessages();
    },
  },
  created() {
    if (!this.userId) return;
    if (!this.talkUserId) return;
    this.setMessages();
  },
  methods: {
    async setMessages() {
      const collectionRef = collection(db, "rooms");
      const q = query(
        collectionRef,
        where("user_ids", "array-contains", this.userId)
      );
      const rooms = await getDocs(q);
      const roomIds = [];
      rooms.forEach((doc) => {
        if (!doc?.id) return;
        const data = doc.data();
        if (!data.user_ids.includes(this.talkUserId)) return;
        roomIds.push(doc.id);
      });
      this.roomId = roomIds[0];
      if (!this.roomId) {
        const docRef = await addDoc(collectionRef, {
          user_ids: [this.userId, this.talkUserId],
        });
        this.roomId = docRef.id;
      }
      const messagesRef = collection(db, "rooms", this.roomId, "messages");
      const messageQ = query(messagesRef, orderBy("send_time"));
      onSnapshot(messageQ, (querySnapshot) => {
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          if (this.messages.some((item) => item.id === doc.id)) return;
          this.messages.push({
            id: doc.id,
            message: data.message,
            name: data.name,
            user_id: data.user_id,
            send_time: data.send_time,
          });
        });
      });
    },
    sendMessage() {
      if (!this.sendMessageText) return;
      if (!this.userName) return;
      if (!this.userId) return;
      const messagesRef = collection(db, "rooms", this.roomId, "messages");
      addDoc(messagesRef, {
        message: this.sendMessageText,
        name: this.userName,
        user_id: this.userId,
        send_time: new Date(),
      });
      this.sendMessageText = "";
    },
  },
});
</script>

<style scoped>
ul li {
  list-style: none;
}
.my-chat-box {
  margin-left: auto;
  background: #faceb3;
  border-radius: 4px;
}
.chat-box {
  display: inline-block;
  position: relative;
  max-width: 300px;
}
.my-chat-box:after {
  content: "";
  position: absolute;
  top: 3px;
  right: -19px;
  border: 8px solid transparent;
  border-left: 18px solid #faceb3;
  -webkit-transform: rotate(-35deg);
  transform: rotate(-35deg);
}

.other-chat-box {
  margin-right: auto;
  border-radius: 4px;
  background: #cacaca;
}

.other-chat-box:after {
  content: "";
  display: inline-block;
  position: absolute;
  top: 3px;
  left: -19px;
  border: 8px solid transparent;
  border-right: 18px solid #cacaca;
  -webkit-transform: rotate(35deg);
  transform: rotate(35deg);
}
.position-relative {
  position: relative;
  overflow: auto;
}
.bottom-bar {
  border-top: 1px solid #cacaca;
  background: #fff;
  position: sticky;
  top: auto;
  bottom: 0;
  left: 0;
  right: 0;
}
</style>
